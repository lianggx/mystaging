...
namespace MyStaging.xUnitTest
{
	public class QueryContextTest
	{
		...
		private static readonly string TestId = "xxx";
		[Fact]
		public void WherePropertyEqualsStaticReadOnlyField()
		{
			int total = 200000;
			var list2 = User.Context.Where(f => f.Id == TestId).ToList();
		...
		public void Count()
		{
			int count = 2;
			// 先把数据库任意两条记录的 age 字段修改为 180 
			var age = User.Context.Where(f => f.Age == 180).GroupBy("age").Count();
		...
		[Fact]
		public void NullableTypeIn()
		{
			bool[] sexs = new[] { true, false };
			var sex = User.Context.Where(f => f.Sex.In(sexs)).ToList();

			Guid[] guids = new[] { Guid.Empty };
			var user = Topic.Context.Where(f => f.User_id.In(guids)).ToList();
		}
	}
}
