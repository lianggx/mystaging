...
namespace MyStaging.Gen.Tool.Generals
{
    public class TableGeneral
    {
        ...
        private readonly List<FieldInfo> fieldList = new List<FieldInfo>();
        private readonly List<PrimarykeyInfo> pkList = new List<PrimarykeyInfo>();
        private readonly List<ConstraintInfo> consList = new List<ConstraintInfo>();
        #endregion

        public TableGeneral(GeneralConfig config, TableViewModel table)
        {
            this.config = config;
            this.table = table;
        }
        ...
        public void CreateModel()
        {
            ...
            using (StreamWriter writer = new StreamWriter(File.Create(_fileName), System.Text.Encoding.UTF8))
            {
                ...
                writer.WriteLine($"using {config.ProjectName}.DAL;");
                ...
                writer.WriteLine("{");
                writer.WriteLine($"\t[EntityMapping(name: \"{this.table.Name}\", Schema = \"{table.Schema}\")]");
                ...
                {
                    string dalPath = $"{ config.ProjectName }.DAL.";
                    ...
                    writer.WriteLine($"\t\t[NonDbColumnMapping, JsonIgnore] public {updateName} UpdateBuilder {{ get {{ return new {updateName}(model =>{{MyStaging.Helpers.MyStagingUtils.CopyProperty<{_classname}>(this, model); SQLHelper.CacheManager?.RemoveItemCache<{_classname}>({cacheString}); }}{dkString}); }} }}");
                    writer.WriteLine();
        ...
        protected void CreateSchema()
        {
            ...
            string _fileName = $"{config.SchemaPath}/{schemaName}.cs";
            using (StreamWriter writer = new StreamWriter(File.Create(_fileName), System.Text.Encoding.UTF8))
            {
                ...
                writer.WriteLine();
                writer.WriteLine($"namespace {config.ProjectName}.Model.Schemas");
                writer.WriteLine("{");
        ...
        protected void CreateDal()
        {
            ...
            string _fileName = $"{config.DalPath}/{_classname}.cs";
            using (StreamWriter writer = new StreamWriter(File.Create(_fileName), System.Text.Encoding.UTF8))
            {
                writer.WriteLine("using System;");
                writer.WriteLine("using System.Linq;");
                ...
                writer.WriteLine($"using {config.ProjectName}.Model.Schemas;");
                writer.WriteLine();
                writer.WriteLine($"namespace {config.ProjectName}.DAL");
                writer.WriteLine("{");
                ...
                {
                    writer.WriteLine();
                    Insert_Generator(writer, _model_classname);
                    writer.WriteLine();
                    Delete_Generator(writer, _model_classname);
                    writer.WriteLine();
                    Update_Generator(writer, _model_classname);
                    writer.WriteLine();
                }
        ...
        protected void Insert_Generator(StreamWriter writer, string class_model)
        {
            string insertName = CreateName();
            writer.WriteLine($"\t\tpublic static InsertBuilder<{class_model}> InsertBuilder => new InsertBuilder<{class_model}>({insertName}Schema.Instance);");
        ...
        protected void Update_Generator(StreamWriter writer, string class_model)
        {
            List<string> d_key = new List<string>();
        ...
        protected void Delete_Generator(StreamWriter writer, string class_model)
        {
            ...
            if (pkList.Count > 0)
            {
                ...
                writer.WriteLine($"\t\t\tif (affrows > 0) SQLHelper.CacheManager?.RemoveItemCache<{class_model}>({cacheString});");
